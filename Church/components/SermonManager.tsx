...
const SermonManager: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, lang?: 'en' | 'zh', field?: 'title' | 'speaker' | 'series' | 'passage') => {
    ...
    if (lang && field) {
        setSermonData(produce(draft => { draft[field][lang] = value; }));
    } else {
        setSermonData(produce(draft => { (draft as any)[name] = value; }));
    }
  };
  ...
  const handleSave = async () => {
    try {
        if (isAdding) {
          ...
          await updateDoc(doc(db, "sermons", editingSermonId), sermonData as any);
        }
        resetForm();
    } catch (e) {
        alert("保存失敗");
