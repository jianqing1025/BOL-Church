...
const detectAspect = (item: VideoItem): Promise<AspectKind> => (
    new Promise((resolve) => {
        const video = document.createElement('video');
        video.preload = 'metadata';
        video.src = item.video;
        video.onloadedmetadata = () => resolve(video.videoWidth >= video.videoHeight ? 'landscape' : 'portrait');
        video.onerror = () => resolve('portrait');
    })
);

const classifyMedia = async (data: VideoItem[]) => {
    const portrait: VideoItem[] = [];
...
export const VideoSlidingTilesSlideshow = ({ onClose, videosData }: { onClose: () => void; videosData: VideoItem[] }) => {
    const [rows, setRows] = useState<SlidingVideoTile[][]>([]);
    ...
    const decksRef = useRef<{ landscape: VideoItem[]; portrait: VideoItem[] }>({ landscape: [], portrait: [] });
    const poolsRef = useRef<{ landscape: VideoItem[]; portrait: VideoItem[] }>({ landscape: [], portrait: [] });
    ...
    const rememberUsage = useCallback((url: string) => {
        ...
        const maxHistory = Math.max(14, Math.min(videosData.length - 1, Math.floor(videosData.length * 0.5)));
        while (recentHistoryRef.current.length > maxHistory) recentHistoryRef.current.shift();
    }, [videosData.length]);

    const getOnScreenUrls = useCallback((currentRows: SlidingVideoTile[][]) => {
        const set = new Set<string>();
        currentRows.forEach((row) => {
            row.forEach((tile) => {
                set.add(tile.top.video);
                if (tile.bottom) set.add(tile.bottom.video);
            });
        });
        return set;
    }, []);
    ...
    const drawItem = useCallback((
        ...
        for (const aspect of tryAspects) {
            ...
            const candidates = deck.filter((item) => !onScreen.has(item.video) && !exclusions.has(item.video) && !recentHistoryRef.current.includes(item.video));
            const fallbackCandidates = deck.filter((item) => !onScreen.has(item.video) && !exclusions.has(item.video));
            const selection = candidates[0] || fallbackCandidates[0];

            if (selection) {
                decksRef.current[aspect] = deck.filter((entry) => entry.video !== selection.video);
                rememberUsage(selection.video);
                return selection;
            }
        }

        const fallback = videosData.find((item) => !exclusions.has(item.video)) || videosData[0];
        rememberUsage(fallback.video);
        return fallback;
    }, [getOnScreenUrls, refillDeck, rememberUsage, videosData]);

    const createTile = useCallback((kind: SlidingTileKind, currentRows: SlidingVideoTile[][], exclusions: Set<string>): SlidingVideoTile => {
        if (kind === 'wide') {
            const top = drawItem('landscape', currentRows, exclusions, true);
            exclusions.add(top.video);
            return { id: makeTileId(), kind, span: 2, top, status: 'idle', travelUnits: 1, travelDirection: -1 };
        }
        ...
        if (kind === 'double') {
            ...
            exclusions.add(bottom.video);
            return { id: makeTileId(), kind, span: 1, top, bottom, status: 'idle', travelUnits: 1, travelDirection: -1 };
        }

        const top = drawItem('portrait', currentRows, exclusions, true);
        exclusions.add(top.video);
        return { id: makeTileId(), kind, span: 1, top, status: 'idle', travelUnits: 1, travelDirection: -1 };
    }, [drawItem]);

    const buildRows = useCallback(() => {
        const nextRows: SlidingVideoTile[][] = [];
        ROW_PATTERNS.forEach((pattern) => {
            const exclusions = new Set<string>();
            nextRows.push(pattern.map((kind) => createTile(kind, nextRows, exclusions)));
        });
        return nextRows;
    }, [createTile]);
    ...
    const triggerRowShift = useCallback((rowIndex?: number, tileIndex?: number) => {
        ...
        setRows((prevRows) => {
            ...
            setFocalContent(removedTile.top.video);
    ...
    const triggerVideoShiftBurst = useCallback(() => {
        if (isAnimatingRef.current || rows.length === 0) return;
        ...
        selectedRows.forEach((rowIndex, index) => {
            window.setTimeout(() => triggerRowShift(rowIndex), index * 260);
        });
    }, [rows, triggerRowShift]);
    ...
    useEffect(() => {
        ...
        void classifyMedia(videosData).then(({ landscape, portrait }) => {
            if (cancelled) return;

            poolsRef.current = {
                landscape: landscape.length ? landscape : videosData,
                portrait: portrait.length ? portrait : videosData,
            };
            decksRef.current = {
                landscape: shuffle([...(landscape.length ? landscape : videosData)]),
                portrait: shuffle([...(portrait.length ? portrait : videosData)]),
            };

            const initialRows = buildRows();
            setRows(initialRows);
            setFocalContent(initialRows[0]?.[0]?.top?.video || videosData[0]?.video || null);
            setReady(true);
        });
    ...
    }, [buildRows, clearTimers, videosData]);

    useEffect(() => {
        if (!ready || !rows.length) return;
        const interval = window.setInterval(() => triggerVideoShiftBurst(), 6000);
        return () => window.clearInterval(interval);
    }, [ready, rows.length, triggerVideoShiftBurst]);
    ...
    return (
        <div ref={shellRef} className="fixed inset-0 z-[100] bg-[#050505] animate-fadeIn flex flex-col overflow-hidden sliding-tiles-shell">
            ...
            {focalContent && (
                <div className="absolute inset-0 z-0 scale-110 transition-all duration-1000">
                    <video crossOrigin="anonymous" src={focalContent} autoPlay muted loop playsInline className="w-full h-full object-cover" style={{ filter: 'blur(70px) brightness(0.26) saturate(1.05)' }} />
                </div>
            )}
                                        ...
                                        {tile.kind === 'double' ? (
                                            <div className="flex flex-col w-full h-full gap-2 p-[2px] bg-black/15">
                                                <div className="min-h-0 flex-1 relative overflow-hidden rounded-[1rem]">
                                                    <VideoStaticTile src={tile.top.video} className="h-full rounded-[1rem]" />
                                                </div>
                                                <div className="min-h-0 flex-1 relative overflow-hidden rounded-[1rem]">
                                                    {tile.bottom && <VideoStaticTile src={tile.bottom.video} className="h-full rounded-[1rem]" />}
                                                </div>
                                            </div>
                                        ) : (
                                            <VideoStaticTile src={tile.top.video} className="h-full rounded-[1.35rem] group-hover/tile:scale-[1.03] transition-transform duration-700" />
                                        )}
                                    </div>
                                );
                            });
