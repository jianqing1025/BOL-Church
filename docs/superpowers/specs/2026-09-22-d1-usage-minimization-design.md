...
### 實作後的成本修正（2026-09-22 總審查）

上表低估了兩處，實作完成後重新量測：

| 項目 | 原估 | 實際 |
| --- | --- | --- |
| 一次 `rebuildSnapshots` | ~4,946 列 | **9,347 列**（`buildSite` 4,947 ＋ `buildCatalogue` 4,400） |
| 重建頻率 | 每日約 6 次 | **每次符合條件的寫入各一次**，不是固定 6 次 |
| 快照重建每日小計 | ~30,000 | ~60,000（估 6 次）；管理員密集編輯 50 次則約 467,000 |
| **合計／日** | ~92,000 | **~120,000** |

仍只佔免費額度 2.4%，結論不變，但數字要誠實。

另有一個原設計沒預料到的反效果：**KV miss 時的 bootstrap 成本是 9,347 列，比改造前的 6,694 還高 40%**（快照建置疊在 bootstrap 自己的查詢之上）。正常情況下 miss 極罕見，不影響結論；但若 KV 寫入持續失敗（binding 設錯、或用盡每日 1,000 次寫入額度），每個請求都會 miss 並重建，反而比不做這個專案更貴。實作已在 `kv.put` 失敗時記錄到 Workers Logs，讓這個狀態不會無聲無息。

### 待決事項：GET 形狀的寫入路徑

`GET /api/live-stream`（`server.ts:3691`）的 heartbeat 會經 `tryArchiveAndNotify` 寫入 `sermons`。這是直播結束後訪客載入 `/live` 時，直播錄影被歸檔成講道的**主要途徑**。

`shouldRebuildSnapshot` 對所有 GET 一律回 false（這是刻意的，否則每次讀取都可能觸發重建），所以這條路徑**在結構上無法被路由清單涵蓋**。兩個選項：

1. 在 `tryArchiveAndNotify` 內部真正寫入成功後，明確呼叫一次 `rebuildSnapshots`
2. 接受新歸檔的直播最多 4 小時後才進入快照（由 cron 收斂）

選項 1 較正確但要處理 Workers 的背景工作生命週期（該處沒有 `ctx`）。**尚未決定，留給階段二處理。** 階段一沒有影響，因為還沒有任何前端讀取目錄。
